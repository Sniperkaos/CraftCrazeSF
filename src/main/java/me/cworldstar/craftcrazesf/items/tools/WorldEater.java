...
public class WorldEater extends SlimefunItem implements Radioactive, DamageableItem, Rechargeable {
    ...
    public void getToolHandler(BlockBreakEvent e,ItemStack tool, int IDK, List<ItemStack> drops) {
          Player p = e.getPlayer();
            if(this.removeItemCharge(tool,2F)) {
                   ...
                   if(b.getBlockData().getMaterial() == Material.STONE && RandomUtils.nextInt(5) >= 4) {
                	   drops.clear();
                	   for(int i=0; i<8; i++) {
                		   drops.add(((ItemStack) Utils.getRandomObject(dusts)).clone());
                	   }
                   }
                    /*if(p.isSneaking()) {
            	...
            	p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&6[CraftCrazeSF]: &dYour tool has run out of power!"));
            	e.setCancelled(true);
            }
    }
